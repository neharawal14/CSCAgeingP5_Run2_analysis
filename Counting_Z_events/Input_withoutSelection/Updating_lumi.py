...
def MergingHV(df_root, df_HV, year) : 
    ...
    merged['int_delivered(/fb)'] =  merged['int_delivered(/fb)'] + delivered_lumi_golden_dict[year]
    merged['int_recorded(/fb)'] =  merged['int_recorded(/fb)']+ recorded_lumi_golden_dict[year]
...
def saveFile(df, year) :
    csv_file = f"/afs/cern.ch/user/n/nrawal/eos/CSCAgeing/Run2_Ntuples/Zfiles/Input_withoutSelection/csc_input_{year}_tree_updatedlumi"
    Savecsv(df, csv_file)
...
if __name__ == "__main__" :
    ...
    year = args.year
    input_name = f"/afs/cern.ch/user/n/nrawal/eos/CSCAgeing/Run2_Ntuples/Zfiles/Input_withoutSelection/csc_input_{year}_tree.root"
